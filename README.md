...
```bash
docker compose -f infra/compose.yaml -f infra/compose.dev.yaml up forum
```
...
|---|---|---|
| **Build & Publish** | `.github/workflows/docker-publish.yml` | Runs `dotnet test`, builds the Docker image, pushes to `ghcr.io/hkarpinen/portfolio-forum:latest` |
| **Deploy** | `.github/workflows/deploy.yml` | Triggers after Build & Publish succeeds; SSHes into the server, pulls the new image, and restarts only the `forum` container |
